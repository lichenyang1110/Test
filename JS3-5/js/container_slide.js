...
function slideAction (targetLeft) {
	...
	timer = setInterval(function () {
		...
		if (parseInt(containerSlideUl.style.left) == targetLeft) {
			...
			isMoving = false;
			//设置当前指示按钮颜色
			setSpanDotColor(parseInt(containerSlideUl.style.left));
		} else {
			//不符合条件，继续移动
...
document.querySelector(".second_bt").onmouseover = function () {
	document.querySelector(".second_bt").style.backgroundPosition = "34% 0";
}
...
////////////////////////////
//传入偏移量，计算当前小圆点指示器
var spans = document.querySelectorAll("#container_slide_button>span");
function setSpanDotColor (offset) {
	for (var i = 0; i < spans.length; i++) {
		spans[i].style.backgroundColor = "rgb(144,144,144)";
	}
	switch (offset){
		case -1226:
